...
# -------------------------------------------------
def scharr_edge(binary_img):
    gx = cv2.Scharr(binary_img.astype(np.float32), cv2.CV_32F, 1, 0)
    gy = cv2.Scharr(binary_img.astype(np.float32), cv2.CV_32F, 0, 1)
    ...
    _, edges = cv2.threshold(mag, 30, 255, cv2.THRESH_BINARY)
