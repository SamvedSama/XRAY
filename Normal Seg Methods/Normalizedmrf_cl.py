...
def mrf_lung_segmentation(img, lambda_smooth=20):
    ...
    h, w = img.shape
    cx, cy = h // 2, w // 2
    center_region = enh_norm[cx - h//6: cx + h//6, cy - w//6: cy + w//6]
    center_mean   = center_region.mean()
    global_mean   = enh_norm.mean()

    lung_label = int(np.argmin(means)) if center_mean < global_mean else int(np.argmax(means))
    ...
    final = morphology.remove_small_holes(final.astype(bool), area_threshold=8001).astype(np.uint8)
    final = morphology.closing(final, morphology.disk(25))
    final = morphology.opening(final, morphology.disk(10))
    ...
    final = binary_fill_holes(clean).astype(np.uint8)

    labels = measure.label(final, connectivity=2)
    props  = sorted(measure.regionprops(labels), key=lambda r: r.area, reverse=True)
    result = np.zeros_like(final)
    for r in props[:2]:
        result[labels == r.label] = 1
...
def process_split_category(split, category):
    ...
    Outputs go to outputs_normal_mrf/<split>/<category>/.
    """
    ...
    with open(metrics_path, "w") as mf:
        ...
        for fname in files:
            try:
                ...
                pred = mrf_lung_segmentation(img)
...
def main():
    print("=" * 60)
    print("  Lung Segmentation — MRF + GMM (Standard_Data)")
